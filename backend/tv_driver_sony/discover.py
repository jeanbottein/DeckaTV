"""Sony BRAVIA LAN discovery via SSDP (UPnP M-SEARCH). Sony-specific, best-effort.

Sony TVs advertise the Scalar Web API service over SSDP, each replying from its own IP,
so multiple sets come back as distinct entries and it needs no extra dependency — just a
few UDP datagrams. A TV in deep standby (Remote Start off) won't answer, so this
complements, never replaces, manual IP entry; any failure yields an empty list.
"""
...
# The search target Sony's Scalar Web API sets answer to.
...
async def discover():
    ...
    except Exception:  # noqa: BLE001 - if we can't even open the socket, discovery is a no-op
        return []
    try:
